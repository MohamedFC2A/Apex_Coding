...
const normalizePath = (rawPath: string) => normalizeWorkspaceFilePath(rawPath);

const normalizeDirPath = (rawPath: string) => normalizeWorkspaceDirectoryPath(rawPath);
      ...
      handleFileEvent: (event) => {
        ...
        if (type === 'start') {
           ...
           if (!exactExists && mode === 'create') {
              ...
              if (existing) {
                 console.warn(`[Anti-Duplication] Redirecting ${path} to ${existing}`);
                 path = existing;
              }
           }
