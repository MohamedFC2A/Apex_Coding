...
const detectWorkspaceRoot = (fileMap) => {
  const paths = Object.keys(fileMap && typeof fileMap === 'object' ? fileMap : {})
    .map((p) => String(p || '').replace(/\\/g, '/').replace(/^\/+/, '').trim())
    .filter(Boolean);

  const has = (target) => paths.includes(target);
  if (has('package.json')) return '';
  if (has('frontend/package.json')) return 'frontend';
  if (has('app/package.json')) return 'app';
  if (has('client/package.json')) return 'client';
...
const scopeCommand = (command, workspaceRoot) => {
  const base = String(command || '').trim();
  if (!base) return base;
  const cwd = String(workspaceRoot || '').trim().replace(/^\/+/, '');
  if (!cwd) return base;
  return `cd ${shellQuote(cwd)} && ${base}`;
};

const withCodeSandboxTasksFile = (fileMap, { devCommand, installCommand, port }) => {
  const next = { ...(fileMap && typeof fileMap === 'object' ? fileMap : {}) };
  if (!next['.codesandbox/tasks.json']) {
    next['.codesandbox/tasks.json'] = JSON.stringify(
      {
        setupTasks: [installCommand || 'npm install'],
        tasks: {
          dev: {
            name: 'Dev Server',
            command: devCommand,
...
const inferDevCommandAndPort = (fileMap) => {
  const workspaceRoot = detectWorkspaceRoot(fileMap);
  const packagePath = workspaceRoot ? `${workspaceRoot}/package.json` : 'package.json';
  const packageJsonRaw =
    (fileMap && typeof fileMap === 'object' && (fileMap[packagePath] || fileMap['package.json'] || fileMap['./package.json'])) || '';
  ...
  if (!packageJsonRaw) {
    ...
    const baseInstallCommand = 'npm install';
    const baseDevCommand = 'npm run dev';
    return {
      installCommand: scopeCommand(baseInstallCommand, workspaceRoot),
      devCommand: scopeCommand(baseDevCommand, workspaceRoot),
      workspaceRoot,
      port: fallbackPort
    };
  }
  ...
  try {
    ...
    if (hasDev && scriptLooksComplex(devScript)) {
      return {
        installCommand: scopeCommand('npm install', workspaceRoot),
        devCommand: scopeCommand('npm run dev', workspaceRoot),
        workspaceRoot,
        port
      };
    }
    if (!hasDev && hasStart && scriptLooksComplex(startScript)) {
      return {
        installCommand: scopeCommand('npm install', workspaceRoot),
        devCommand: scopeCommand('npm start', workspaceRoot),
        workspaceRoot,
        port
      };
    }
    ...
    if (hasDev) {
      if (hasNext) {
        ...
        const baseDevCommand = extra.length > 0 ? `npm run dev -- ${extra.join(' ')}` : 'npm run dev';
        return {
          installCommand: scopeCommand('npm install', workspaceRoot),
          devCommand: scopeCommand(baseDevCommand, workspaceRoot),
          workspaceRoot,
...
const createSandboxPreview = async ({ fileMap, timeoutMs = 300000 }) => {
  let sdk;
  try {
    sdk = getSdk();
  } catch (error) {
  ...
  const { installCommand, devCommand, port } = inferDevCommandAndPort(fileMap);
  const effectiveFileMap = withCodeSandboxTasksFile(fileMap, { installCommand, devCommand, port });
  ...
  try {
    ...
    await client.commands.run(installCommand || 'npm install', { name: installCommand || 'npm install' }).catch(() => {});
...
const patchSandboxFiles = async ({
  ...
  try {
    ...
    const { installCommand, devCommand, port } = inferDevCommandAndPort(inferredMap);
